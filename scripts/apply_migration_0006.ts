...
async function run() {
  console.log(`Aplicando migración 0006 usando entorno: ${envPath}...`);
  const dbUrl = process.env.DATABASE_URL;
  if (!dbUrl) throw new Error("DATABASE_URL no definida");

  const sql = postgres(dbUrl, { max: 1 });

  // 1. Tipo estado_suscripcion seguro
  await sql`
    DO $$ BEGIN
    ...
    END $$;
  `;
  console.log("✓ Tipo estado_suscripcion verificado/creado.");
  ...
  `;
  console.log("✓ Tabla registros_pendientes verificada/creada.");

  // 3. Tabla stripe_eventos_procesados
  await sql`
  ...
  `;
  console.log("✓ Tabla stripe_eventos_procesados verificada/creada.");

  // 4. Columnas en restaurantes
  await sql`
  ...
  `;
  console.log("✓ Columnas en restaurantes verificadas/creadas.");

  await sql.end();
  console.log("✓ Migración 0006 finalizada con éxito.");
  process.exit(0);
}

run().catch((err) => {
  console.error("Error aplicando migración 0006:", err);
  process.exit(1);
});
